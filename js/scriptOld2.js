    ...
    for (var i = 0; i < options.length; i++) {
        ...
        for (var j = 0; j < options[i].length; j++) {
            ...
            if (targetId === options[i][j]) {
                //replace the cell with the letter in the arrays
  ...
  for(var i= 1; i< options.length; i++){
    ...
    for(var j= options[i].length; j>1; j--){
      ...
      if(options[i][j]==options[i][j-1]){
        ...
        //options[i].splice(options[i].indexOf(picked), 1, o);
         return picked;
      }else {
        console.log("else");
        //put options in case there is two of the same O
        //if nothing else, put in the first free cell
        //need to replace the cell with o in the right position, also with chooseCell()
      }
    }
  }
  //console.log(options);
